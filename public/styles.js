/* VARIÁVEIS BASE (idênticas ao Pedido de Faturamento) */
:root {
    --primary: #ff521d;
    --bg-primary: #FFFFFF;
    --bg-secondary: #F8F9FA;
...
@media (prefers-color-scheme: dark) {
    :root {
        ...
        --border-color: rgba(255, 82, 29, 0.08);
        --input-bg: #2A2A2A;
        --table-stripe: #1F1F1F;
        --card-shadow: none;
        --th-bg: #4A4A4A;
...
/* BOTÃO DE NOVO PEDIDO (mantido, mesmo não usado) */
...
.filter-dropdown-inline select:hover {
    border-color: var(--text-secondary);
}

.filter-dropdown-inline select:focus {
    outline: none;
    border-color: var(--primary);
}

.filter-dropdown-inline .dropdown-icon {
    position: absolute;
    right: 0.75rem;
    top: 50%;
...
/* BOTÕES DE AÇÃO (CALENDÁRIO, SINCRONIZAR, RELATÓRIO) */
.calendar-btn {
    background: transparent;
    border: none;
    color: var(--text-secondary);
...
.calendar-month:hover {
    ...
    box-shadow: 0 4px 8px rgba(255, 82, 29, 0.3);
}
...
.badge.aberta {
    background: rgba(255, 82, 29, 0.1);
    color: #ff521d;
    border: 1px solid rgba(255, 82, 29, 0.2);
}
...
/* FORMULÁRIOS E INPUTS (para modais) */
.form-grid {
    display: grid;
...
button.danger, .btn-delete {
    background: var(--btn-delete);
}

button.success, .btn-save-green {
